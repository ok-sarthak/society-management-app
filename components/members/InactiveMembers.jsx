    ...
    >
      <View style={styles.modalContainer}>
        <View style={styles.modalHeader}>
          <Text style={styles.modalTitle}>Inactive Member Details</Text>
  ...
  return (
    <View style={styles.detailItem}>
      <View style={styles.detailHeader}>
        <Ionicons name={icon} size={16} color="#dc3545" />
...
const InactiveMemberCard = ({ member, onPress }) => {
  return (
      ...
      <View style={styles.cardHeader}>
        ...
        <View style={styles.cardRight}>
          <View style={styles.inactiveBadgeSmall}>
            <Text style={styles.inactiveTextSmall}>INACTIVE</Text>
          </View>
...
const InactiveMembers = ({ onClose }) => {
  ...
  const loadInactiveMembers = async () => {
    setLoading(true);
    try {
      const result = await membersService.getInactiveMembers();
      if (result.success) {
        setMembers(result.data);
      } else {
        Alert.alert('Error', result.error || 'Failed to load inactive members');
      }
    } catch (_error) {
  ...
  const renderMember = ({ item }) => (
    <InactiveMemberCard member={item} onPress={handleMemberPress} />
  );

  const renderEmptyState = () => (
    <View style={styles.emptyState}>
      <Ionicons name="close-circle-outline" size={64} color="#dc3545" />
      <Text style={styles.emptyStateTitle}>
        {searchQuery ? 'No inactive members found' : 'No inactive members'}
      </Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Inactive Members ({filteredMembers.length})</Text>
        <TouchableOpacity onPress={onClose} style={styles.closeButton}>
          <Ionicons name="close" size={24} color="#666" />
        </TouchableOpacity>
      </View>
      ...
      {loading ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading inactive members...</Text>
        </View>
      ) : (
        <FlatList
